...
export default function ProductCard({ product, sectionKey }) {
  ...
  {/* Nom du produit */}
  <h3 className={`font-display text-2xl font-bold text-center mb-1${['Vermicelle', 'Couscous'].includes(product.name) ? ' text-black' : ' text-ink'}`}>{product.name}</h3>
      {/* Description concise */}
      ...
      <div className="flex justify-center gap-2 mt-4 mb-6">
        <button
          ...
          className={`btn${unavailable ? ' btn-disabled bg-gray-300 text-gray-500' : ' bg-gray-200 text-black rounded-lg hover:bg-gray-300 transition'}`}
        >
          Détails
        </button>
        <button
